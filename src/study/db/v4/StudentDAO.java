...
//		TO_CHAR(birthday, 'YYYY-MM-DD') 
		...
		try {
			...
			if (rs.next()) { // 읽어온 데이터를 행 단위로 반복하면서 접근
				...
				s = new StudentMypageDTO(rs.getInt("STUDNO"), rs.getString("NAME"), rs.getString("ID"),
						...
						rs.getInt("PROFNO"),rs.getInt("deptno"),rs.getString("dname"),rs.getInt("part"),rs.getString("build"));
	...
	public Student findStudentByStudno(int studno) {
		...
		String query = "select studno,name,id,grade,jumin "
				+ " ,TO_CHAR(birthday, 'YYYY-MM-DD') birthday, tel,height,weight,deptno1,deptno2,profno from student "
				+ " where studno = ? ";
...
//		TO_CHAR(birthday, 'YYYY-MM-DD') 
		...
		try {
			...
			if (rs.next()) { // 읽어온 데이터를 행 단위로 반복하면서 접근
				...
				s = new Student(rs.getInt("STUDNO"), rs.getString("NAME"), rs.getString("ID"),
						rs.getInt("GRADE"), rs.getString("JUMIN"), rs.getString("BIRTHDAY"), rs.getString("TEL"),
						rs.getInt("HEIGHT"), rs.getInt("WEIGHT"), rs.getInt("DEPTNO1"), rs.getInt("DEPTNO2"),
						rs.getInt("PROFNO"));
